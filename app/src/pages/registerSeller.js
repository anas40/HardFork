...
export default function RegisterSeller() {
  ...
  const schema = yup.object({
    name: yup
      .string()
      .required("Required!")
      .max(50, "ProductName Should be less than 250 characters")
      .test("no Num", "Number not allowed", async (val) => {
        if (val) {
          return await !val.match(/[0-9]+/);
        }
        return false;
      })
      .test("noSpecial", "Special characters not allowed", async (val) => {
        if (val) {
          return await val.match(/[a-z]/i);
        }
        return false;
      }),
  ...
  async function register(values) {
    try {
      ...
      await provider.sendTransaction("registerSeller", [
        values.name,
        values.details,
      ]);
